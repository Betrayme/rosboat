...
int servinteract(const char* argv1,const char* argv2,const char* argv3);
int servinteract(const char* argv1,const char* argv2);
/**************************************************TCP****************************************************/
class TCPServer4{
public:
    string ip;
...
inline TCPServer4::TCPServer4(const char* IP,const char* port)
{
    ...
    serv_addr.sin_addr.s_addr = inet_addr(IP);    // inet_aton(IP, &serv_addr.sin_addr);
    ip = inet_ntoa(serv_addr.sin_addr);  //获取服务器的ip并存储
    //  serv_addr.sin_addr.s_addr=htonl(INADDR_ANY);    //设置存放服务器地址端口等信息的结构体的IP为自动分配
